...
#'
#' @return A LASERF tree object containing the below attributes.
...
get_tree <- function(forest, index) {
  ...
  # Convert internal laserf representation to adjacency list.
  # +1 from C++ to R index.
...
#'
#' Given a LASERF tree object, compute the leaf node a test sample falls into. The nodes in a LASERF tree
...
#'
#' @param tree A LASERF tree object (retrieved by `get_tree`).
...
get_leaf_node <- function(tree, newdata, node.id = TRUE) {
  if (!("laserf_tree" %in% class(tree))) {
    stop("get_leaf_node is only implemented for class `laserf_tree`")
  }
...
#' @param forest The LASERF forest
#' @param samples The samples to include in the calculations.
#' @param ... Additional arguments (currently ignored).
#'
